...
(function(mod) {
    ...
    DiffView.prototype = {
        ...
        init: function(pane, orig, options) {
            ...
            if (
                Object.prototype.toString.call(classLocation) !=
                '[object Array]'
            )
    ...
    function markChanges(editor, diff, type, marks, from, to, classes) {
        ...
        function markChunk(start, end) {
            ...
            if (start == end && bfrom == end && bto == end) {
                if (bfrom)
                    marks.push(
                        addClass(editor, bfrom - 1, classes, false, false, true)
                    );
                else
                    marks.push(
    ...
    function drawConnectorsForChunk(dv, chunk, sTopOrig, sTopEdit, w) {
        ...
        var top =
            dv.orig.heightAtLine(chunk.origFrom, 'local', true) - sTopOrig;
        if (dv.svg) {
            var topLpx = top;
            var topRpx =
            ...
            var botLpx =
                dv.orig.heightAtLine(chunk.origTo, 'local', true) - sTopOrig;
            var botRpx =
                dv.edit.heightAtLine(chunk.editTo, 'local', true) - sTopEdit;
            if (flip) {
                var tmp = botLpx;
                botLpx = botRpx;
                botRpx = tmp;
    ...
    function copyChunk(dv, to, from, chunk) {
        ...
        to.replaceRange(
            from.getRange(origStart, Pos(chunk.origTo, 0)),
            editStart,
            Pos(chunk.editTo, 0)
        );
    }
