        ...
        for (;;) {
            if (
                ...
                if (
                    toOutside &&
                    !bidi &&
                    !/\s/.test(lineObj.text.charAt(ch)) &&
                    xDiff > 0 &&
                    ch < lineObj.text.length &&
                    preparedMeasure.view.measure.heights.length > 1
                ) {
                    var charSize = measureCharPrepared(
                        cm,
                        preparedMeasure,
                        ch,
                        'right'
                    ...
                    if (
                        innerOff <= charSize.bottom &&
                        innerOff >= charSize.top &&
                        Math.abs(x - charSize.right) < xDiff
                    ) {
                        outside = false;
                        ch++;
    ...
    function leftButtonSelect(cm, e, start, type, addNew) {
        ...
        function extendTo(pos) {
            ...
            if (type == 'rect') {
                ...
                setSelection(
                    doc,
                    normalizeSelection(ranges$1, ourIndex),
                    sel_mouse
                );
            }
        }
        ...
        CodeMirror.prototype = {
            ...
            getHelpers: function(pos, type) {
                ...
                for (var i$1 = 0; i$1 < help._global.length; i$1++) {
                    ...
                    if (
                        cur.pred(mode, this$1) &&
                        indexOf(found, cur.val) == -1
                    ) {
                        found.push(cur.val);
                    }
                }
            ...
            scrollIntoView: methodOp(function(range, margin) {
                ...
                if (range.from.line != null) {
                    ...
                    var sPos = calculateScrollPos(
                        ...
                        Math.max(range.from.bottom, range.to.bottom) +
                            range.margin
                    );
